  ...
  class Renderer{
    ...
    labels(world){const c=this.c,p=world.player;const crowded=world.enemies.length>45;const near=[...world.enemies].sort((a,b)=>Math.hypot(a.x-p.x,a.y-p.y)-Math.hypot(b.x-p.x,b.y-p.y));
      for(let i=0;i<near.length;i++){const e=near[i],pos=this.screen(e.x,e.y),r=e.r*this.cam.scale;if(r<7||r>350||pos.x<-90||pos.x>this.w+90||pos.y<-50||pos.y>this.h+50)continue;if(crowded&&i>30&&!e.boss)continue;
        const strong=e.level>world.save.level*1.55,equal=e.level>world.save.level,color=strong?'#bf6855':equal?'#947848':'#537b55';const text=(e.boss?'♛ ':'')+e.level.toLocaleString('ru-RU'),fs=e.boss?13:11;c.font=`800 ${fs}px Segoe UI`;c.textAlign='center';const width=c.measureText(text).width+17,x=pos.x,y=pos.y-r*2.05;
        ...
        if(e.boss&&r>20){c.font='10px Segoe UI';c.strokeStyle='#142b28';c.lineWidth=3;c.strokeText(e.name,x,y-20);c.fillStyle='#f8f4e2';c.fillText(e.name,x,y-20);}
      }
      const here=this.screen(p.x,p.y),r=p.r*this.cam.scale;c.textAlign='center';c.font='900 19px Segoe UI';const text='УР. '+world.save.level.toLocaleString('ru-RU');c.strokeStyle='#397362';c.lineWidth=4;c.strokeText(text,here.x,here.y-r*2.3);c.fillStyle='#fffce4';c.fillText(text,here.x,here.y-r*2.3);
